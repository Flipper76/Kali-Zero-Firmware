    ...
    do {
        ...
        if(((!strcmp(furi_string_get_cstr(temp_str), SUBGHZ_KEY_FILE_TYPE)) ||
           ...
           temp_data32 == SUBGHZ_KEY_FILE_VERSION) {
            ...
            FURI_LOG_E(TAG, "Discordance de type/version");
            break;
        }
        ...
        if(!flipper_format_read_uint32(fff_data_file, "Frequency", &temp_data32, 1)) {
            FURI_LOG_W(TAG, "Impossible de lire la fréquence. Définir par défaut");
            ...
            FURI_LOG_E(TAG, "Fréquence non prise en charge sur le module radio choisi");
            break;
        }
        ...
        if(protocol->flag & SubGhzProtocolFlag_Send) {
            if((protocol->type == SubGhzProtocolTypeStatic) ||
               ...
               (protocol->type == SubGhzProtocolTypeRAW)) {
                ...
                FURI_LOG_E(TAG, "Protocole non supporté");
                break;
            }

            furi_string_set(sub_preset->protocaol_name, temp_str);
        } else {
            FURI_LOG_E(TAG, "Le protocole ne supporte pas la transmission");
            break;
        }
...
#if FURI_DEBUG
        FURI_LOG_I(TAG, "%-16s - protocole chargé", furi_string_get_cstr(sub_preset->label));
#endif
    } while(false);
