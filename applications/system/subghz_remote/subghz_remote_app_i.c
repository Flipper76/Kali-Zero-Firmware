...
static SubRemLoadMapState subrem_map_preset_check(
    ...
    for(uint8_t i = 0; i < SubRemSubKeyNameMaxCount; i++) {
        ...
        if(furi_string_empty(sub_preset->file_path)) {
            ...
            FURI_LOG_W(TAG, "Erreur ouverture fichier %s", furi_string_get_cstr(sub_preset->file_path));
        } else {
...
#if FURI_DEBUG
            FURI_LOG_W(TAG, "Aucun chemin de fichier pour %s", map_file_labels[i][0]);
#endif
            sub_preset->type = SubGhzProtocolTypeUnknown;
        } else if(!path_contains_only_ascii(furi_string_get_cstr(sub_preset->file_path))) {
            FURI_LOG_E(TAG, "Caractères incorrects dans le chemin du fichier [%s]", map_file_labels[i][0]);
            sub_preset->type = SubGhzProtocolTypeUnknown;
...
#if FURI_DEBUG
            FURI_LOG_W(TAG, "Aucun label pour %s", map_file_labels[i][0]);
#endif
            ret = true;
        } else {
            ret = true;
...
SubRemLoadMapState subrem_map_file_load(SubGhzRemoteApp* app, const char* file_path) {
    ...
    if(ret == SubRemLoadMapStateOK) {
        FURI_LOG_I(TAG, "Chargement réussi du fichier Map");
    } else if(ret == SubRemLoadMapStateNotAllOK) {
        FURI_LOG_I(TAG, "Chargement du fichier Map en toute sécurité [Pas tous les fichiers]");
    } else {
        FURI_LOG_E(TAG, "Fichier Map cassé");
    }
...
bool subrem_tx_start_sub(SubGhzRemoteApp* app, SubRemSubFilePreset* sub_preset) {
    ...
    if(sub_preset->type == SubGhzProtocolTypeUnknown) {
        ...
        FURI_LOG_I(TAG, "Envois %s", furi_string_get_cstr(sub_preset->label));
