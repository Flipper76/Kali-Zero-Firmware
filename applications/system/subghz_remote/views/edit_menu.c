...
void subrem_view_edit_menu_draw(Canvas* canvas, SubRemViewEditMenuModel* model) {
    ...
    elements_button_center(canvas, "Editer");
    elements_button_right(canvas, "Enreg.");
    ...
    case SubRemSubKeyNameUp:
        canvas_draw_str(canvas, 3, FRAME_HEIGHT - 2, "HAUT");
        break;

    case SubRemSubKeyNameDown:
        canvas_draw_str(canvas, 3, FRAME_HEIGHT - 2, "BAS");
        break;

    case SubRemSubKeyNameLeft:
        canvas_draw_str(canvas, 3, FRAME_HEIGHT - 2, "GAUCHE");
        break;

    case SubRemSubKeyNameRight:
        canvas_draw_str(canvas, 3, FRAME_HEIGHT - 2, "DROITE");
        break;
    ...
    elements_text_box(
        ...
        AlignCenter,
        AlignBottom,
        furi_string_empty(model->label) ? "Label vide" : furi_string_get_cstr(model->label),
        true);
    ...
    if(model->sub_state == SubRemLoadSubStateOK) {
        ...
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str(canvas, 1, FRAME_HEIGHT * 2 - 2, "Bouton non config.");
    } else {
        canvas_set_font(canvas, FontPrimary);
