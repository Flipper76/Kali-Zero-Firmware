...
static bool update_task_remove_stack(UpdateTask* update_task) {
    do {
        FURI_LOG_W(TAG, "Suppression pile");
...
static bool update_task_manage_radiostack(UpdateTask* update_task) {
    ...
    do {
        ...
        if(c2_state->mode == BleGlueC2ModeStack) {
            ...
            FURI_LOG_W(TAG, "Attends l’installation FUS");
            ...
            if(stack_version_match) {
                ...
                if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagC2Update)) {
                    ...
                    furi_hal_rtc_reset_flag(FuriHalRtcFlagC2Update);
                    FURI_LOG_W(TAG, "Forcer suppression de pile (correspondance)");
                    ...
                    FURI_LOG_W(TAG, "Lancer la pile pour tester la version");
...
bool update_task_validate_optionbytes(UpdateTask* update_task) {
    ...
    for(size_t idx = 0; idx < FURI_HAL_FLASH_OB_TOTAL_VALUES; ++idx) {
        ...
        if(ref_value != device_ob_value_masked) {
            ...
            FURI_LOG_E(
                TAG,
                "Discordance OB : #%d : réel %08lX != %08lX (exp.), complet %08lX",
                idx,
            ...
            if(can_patch) {
                ...
                FURI_LOG_W(TAG, "Correction de l'octet OB #%d à %08lX", idx, patched_value);
                ob_dirty = true;
                ...
                if(!is_fixed) {
                    ...
                    FURI_LOG_W(
                        TAG,
                        "OB #%d est FUBAR (fixé et masqué %08lX, pas %08lX)",
                        idx,
                        patched_value,
                        ref_value);
                }
            ...
            FURI_LOG_D(
                TAG,
                "Correspondance OB : #%d : réel %08lX == %08lX (exp.)",
                idx,
                device_ob_value_masked,
                ref_value);
        }
    ...
    if(ob_dirty) {
        FURI_LOG_W(TAG, "OB modifiés, application");
