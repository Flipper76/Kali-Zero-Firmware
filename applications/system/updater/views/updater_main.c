...
static void updater_main_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->failed) {
        ...
        canvas_draw_str_aligned(canvas, 40, 9, AlignLeft, AlignTop, "Echec de la MAJ!");
        ...
        canvas_draw_str_aligned(
            canvas, 18, 55, AlignLeft, AlignTop, "pour retenter, maintenez       pour annuler");
        canvas_draw_icon(canvas, 7, 54, &I_Ok_btn_9x9);
        canvas_draw_icon(canvas, 75, 55, &I_Pin_back_arrow_10x8);
    } else {
        canvas_draw_str_aligned(canvas, 82, 16, AlignCenter, AlignTop, "Kali Zero");
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 82, 6, AlignCenter, AlignTop, "Installation");
        canvas_draw_str_aligned(
...
UpdaterMainView* updater_main_alloc(void) {
    ...
    with_view_model(
        main_view->view,
        UpdaterProgressModel * model,
        { model->status = furi_string_alloc_set("Attends une carte SD"); },
        true);
