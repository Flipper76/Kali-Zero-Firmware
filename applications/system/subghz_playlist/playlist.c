...
static int playlist_worker_process(
    ...
    if(!flipper_format_file_open_existing(fff_file, path)) {
        FURI_LOG_E(TAG, "  (TX) Impossible d'ouvrir %s", path);
        return -1;
    }
    ...
    if(!subghz_devices_set_tx(worker->radio_device)) {
        FURI_LOG_E(
            TAG,
            "  (TX) L'appareil SubGhz utilisé ne supporte pas la fréquence  %lu",
            frequency);
        return -5;
    }
    FURI_LOG_D(TAG, "  (TX) Début d'envoie...");
    int status = 0;
    ...
    while(!subghz_devices_is_async_complete_tx(worker->radio_device)) {
        if(worker->ctl_request_exit) {
            FURI_LOG_D(TAG, "    (TX) Sortie demandé. Annuler l'envoi...");
            status = 2;
            break;
        }
        if(worker->ctl_pause) {
            FURI_LOG_D(TAG, "    (TX) Pause demandé. Annulation et renvoi...");
            status = 1;
            break;
        }
        if(worker->ctl_request_skip) {
            worker->ctl_request_skip = false;
            FURI_LOG_D(TAG, "    (TX) Passer demandé. Annulation et renvoi...");
            status = 0;
            break;
        }
        if(worker->ctl_request_prev) {
            worker->ctl_request_prev = false;
            FURI_LOG_D(TAG, "    (TX) Retour demandé. Annulation et renvoi...");
            status = 3;
            break;
        }
        furi_delay_ms(50);
...
static bool playlist_worker_wait_pause(PlaylistWorker* worker) {
    ...
    if(worker->ctl_request_exit) {
        FURI_LOG_D(TAG, "Sortie demandé. Sortie de boucle...");
        return false;
    }
    return true;
}
...
static bool playlist_worker_play_playlist_once(
    ...
    if(!flipper_format_rewind(fff_head)) {
        FURI_LOG_E(TAG, "Impossible de rembobiner le fichier");
        return false;
    }
    ...
    while(flipper_format_read_string(fff_head, "sub", data)) {
        ...
        for(int i = 0; i < 1; i++) {
            ...
            FURI_LOG_D(TAG, "(worker) Envoi %s", str);
...
static int32_t playlist_worker_thread(void* ctx) {
    ...
    if(!flipper_format_file_open_existing(fff_head, furi_string_get_cstr(worker->file_path))) {
        FURI_LOG_E(TAG, "Echec d'ouverture %s", furi_string_get_cstr(worker->file_path));
    ...
    for(int i = 0; i < MAX(1, worker->meta->playlist_repetitions); i++) {
        ...
        FURI_LOG_D(
            TAG,
            "Envoi de la playlist (i %d rep %d b %d)",
            i,
            worker->meta->current_playlist_repetition,
    ...
    FURI_LOG_D(TAG, "Fin de lecture. Lire %d lignes de données.", worker->meta->current_count);
    worker->is_running = false;
...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    case STATE_OVERVIEW:
        ...
        // draw loaded count
        {
            furi_string_printf(temp_str, "%d Items dans playlist", app->meta->total_count);
            canvas_draw_str_aligned(
...
int32_t playlist_app(char* p) {
    // create playlist folder
    {
        ...
        if(!storage_simply_mkdir(storage, PLAYLIST_FOLDER)) {
            FURI_LOG_E(TAG, "Impossible de créer le dossier %s", PLAYLIST_FOLDER);
        }
        furi_record_close(RECORD_STORAGE);
    }
    ...
    if(p && strlen(p)) {
        ...
        if(!res) {
            FURI_LOG_E(TAG, "Aucune sélection de fichier");
            goto exit_cleanup;
        }
    }
    ...
    while(1) { // close application if no file was selected
        ...
        case InputKeyOk:
            ...
            break;
        case InputKeyBack:
            FURI_LOG_D(TAG, "Appui sur Retour. L'app va se fermer");
            exit_loop = true;
            break;
        default:
            break;
...
exit_cleanup:
    ...
    if(app->worker != NULL) {
        if(playlist_worker_running(app->worker)) {
            FURI_LOG_D(TAG, "Flux toujours exécuté. Demande de fin du flux ...");
            playlist_worker_stop(app->worker);
        }
        FURI_LOG_D(TAG, "Libérer le travailleur ...");
        playlist_worker_free(app->worker);
    }

    FURI_LOG_D(TAG, "Libérer la playlist...");
    playlist_free(app);
    return 0;
}
