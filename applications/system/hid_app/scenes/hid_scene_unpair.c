...
void hid_scene_unpair_on_enter(void* context) {
    ...
    dialog_ex_set_header(app->dialog, "Dissocier les appareils?", 64, 3, AlignCenter, AlignTop);
    dialog_ex_set_text(
        app->dialog, "Les appairagess\nseront perdus!", 64, 22, AlignCenter, AlignTop);
    dialog_ex_set_left_button_text(app->dialog, "Retour");
    dialog_ex_set_right_button_text(app->dialog, "Dissocier");

    // Un-pair success popup view
    popup_set_icon(app->popup, 48, 6, &I_DolphinDone_80x58);
    popup_set_header(app->popup, "Fait", 14, 15, AlignLeft, AlignTop);
