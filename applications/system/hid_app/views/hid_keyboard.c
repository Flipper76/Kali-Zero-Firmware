...
const HidKeyboardKey hid_keyboard_keyset[ROW_COUNT][COLUMN_COUNT] = {
	...
	{
        {.width = 2, .icon = &I_Ctrl_17x10, .icon_toggled = &I_Ctrl_pressed_17x10, .value = HID_KEYBOARD_L_CTRL},
        {.width = 0, .value = HID_KEYBOARD_L_CTRL},
        {.width = 2, .icon = &I_Alt_17x10, .icon_toggled = &I_Alt_pressed_17x10,.value = HID_KEYBOARD_L_ALT},
        {.width = 0, .value = HID_KEYBOARD_L_ALT},
        {.width = 2, .icon = &I_Cmd_17x10, .icon_toggled = &I_Cmd_pressed_17x10,.value = HID_KEYBOARD_L_GUI},
...
static void hid_keyboard_draw_callback(Canvas* canvas, void* context) {
    ...
    if((!model->connected) && (model->transport == HidTransportBle)) {
        ...
        canvas_draw_icon(canvas, 55, 3, &I_Pin_back_arrow_10x8);
        canvas_set_font(canvas, FontSecondary);
        elements_multiline_text_aligned(canvas, 127, 4, AlignRight, AlignTop, "Tenir=Quitter");
