...
void hex_viewer_startscreen_draw(Canvas* canvas, HexViewerStartscreenModel* model) {
    ...
    if(!model->file_size) {
        ...
        canvas_draw_str_aligned(canvas, 64, 22, AlignCenter, AlignTop, "Visionneuse hex de base");
        canvas_draw_str_aligned(canvas, 64, 32, AlignCenter, AlignTop, "pour votre Flipper");
        elements_button_center(canvas, "Open");
    } else {
        canvas_set_color(canvas, ColorBlack);

        elements_button_left(canvas, model->mode ? "Adresse" : "Texte");
