...
void hex_viewer_save_settings(void* context) {
    ...
    FURI_LOG_D(TAG, "Enregistrement configuration");
    ...
    if(!storage_common_stat(storage, HEX_VIEWER_SETTINGS_SAVE_PATH, NULL) == FSE_OK) {
        FURI_LOG_D(
            TAG, "Le fichier de configuration %s est introuvable. Il en créera un nouveau.", HEX_VIEWER_SETTINGS_SAVE_PATH);
        if(storage_common_stat(storage, CONFIG_FILE_DIRECTORY_PATH, NULL) == FSE_NOT_EXIST) {
            FURI_LOG_D(
                TAG, "Le répertoire %s n'existe pas. Il en créera un nouveau.", CONFIG_FILE_DIRECTORY_PATH);
            if(!storage_simply_mkdir(storage, CONFIG_FILE_DIRECTORY_PATH)) {
                FURI_LOG_E(TAG, "Erreur pendant la création du répertoire %s", CONFIG_FILE_DIRECTORY_PATH);
            }
        }
    }
    ...
    if(!flipper_format_file_open_new(fff_file, HEX_VIEWER_SETTINGS_SAVE_PATH)) {
        ...
        FURI_LOG_E(TAG, "Erreur pendant la création d'un nouveau fichier %s", HEX_VIEWER_SETTINGS_SAVE_PATH);
    ...
    if(!flipper_format_rewind(fff_file)) {
        ...
        FURI_LOG_E(TAG, "Erreur de retour");
        hex_viewer_close_storage();
        return;
    }
...
void hex_viewer_read_settings(void* context) {
    ...
    if(!flipper_format_file_open_existing(fff_file, HEX_VIEWER_SETTINGS_SAVE_PATH)) {
        FURI_LOG_E(TAG, "Impossible d'ouvrir le fichier %s", HEX_VIEWER_SETTINGS_SAVE_PATH);
        hex_viewer_close_config_file(fff_file);
        hex_viewer_close_storage();
        return;
    }
    ...
    if(!flipper_format_read_header(fff_file, temp_str, &file_version)) {
        FURI_LOG_E(TAG, "Données header manquantes");
        hex_viewer_close_config_file(fff_file);
        hex_viewer_close_storage();
    ...
    if(file_version < HEX_VIEWER_SETTINGS_FILE_VERSION) {
        FURI_LOG_I(TAG, "ancienne version de configuration, sera supprimée.");
        hex_viewer_close_config_file(fff_file);
        hex_viewer_close_storage();
        return;
    }
...
bool hex_viewer_open_file(void* context, const char* file_path) {
    ...
    do {
        if(!buffered_file_stream_open(
            ...
            FURI_LOG_E(TAG, "Impossible d'ouvrir le flux: %s", file_path);
            isOk = false;
            break;
        };
...
bool hex_viewer_read_file(void* context) {
    ...
    do {
        ...
        if(!stream_seek(hex_viewer->model->stream, offset, true)) {
            FURI_LOG_E(TAG, "Impossible de rechercher le flux");
            isOk = false;
            break;
        }
