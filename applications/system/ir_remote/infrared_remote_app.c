...
static void app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    if(app->status) {
        ...
        canvas_draw_str_aligned(canvas, 62, 5, AlignCenter, AlignTop, "config. incorrecte.");
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 62, 30, AlignCenter, AlignTop, "Configurer la carte.");
        canvas_draw_str_aligned(canvas, 62, 60, AlignCenter, AlignBottom, "Retour pour quitter.");
    } else {
        canvas_clear(canvas);
        ...
        canvas_draw_str_aligned(canvas, 32, 123, AlignCenter, AlignCenter, "Quitter");
    }
}
...
int32_t infrared_remote_app(char* p) {
    ...
    if(!flipper_format_file_open_existing(ff, furi_string_get_cstr(map_file))) {
        FURI_LOG_E(TAG, "Impossible d'ouvrir le fichier MAP %s", furi_string_get_cstr(map_file));
        app->status = 1;
    } else {
        //Filename Assignment/Check Start

        if(!flipper_format_read_string(ff, "REMOTE", remote_path)) {
            FURI_LOG_E(TAG, "Impossible de lire la chaîne REMOTE");
            app->status = 1;
        } else {
            if(!infrared_remote_load(remote, remote_path)) {
                FURI_LOG_E(TAG, "Impossible de charger le fichier ir: %s", furi_string_get_cstr(remote_path));
                app->status = 1;
            } else {
                FURI_LOG_I(TAG, "Fichier REMOTE chargé: %s", furi_string_get_cstr(remote_path));
            }
        }
        ...
        if(!flipper_format_read_string(ff, "DOWN", app->down_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne DOWN");
        ...
        if(!flipper_format_read_string(ff, "LEFT", app->left_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne LEFT");
        ...
        if(!flipper_format_read_string(ff, "RIGHT", app->right_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne RIGHT");
        ...
        if(!flipper_format_read_string(ff, "OK", app->ok_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne OK");
        ...
        if(!flipper_format_read_string(ff, "BACK", app->back_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne BACK");
        ...
        if(!flipper_format_read_string(ff, "UPHOLD", app->up_hold_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne UPHOLD");
        ...
        if(!flipper_format_read_string(ff, "DOWNHOLD", app->down_hold_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne DOWNHOLD");
        ...
        if(!flipper_format_read_string(ff, "LEFTHOLD", app->left_hold_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne LEFTHOLD");
        ...
        if(!flipper_format_read_string(ff, "RIGHTHOLD", app->right_hold_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne RIGHTHOLD");
        ...
        if(!flipper_format_read_string(ff, "OKHOLD", app->ok_hold_button)) {
            FURI_LOG_W(TAG, "Impossible de lire la chaîne OKHOLD");
                ...
                if(event.type == InputTypeShort) {
                    ...
                    case InputKeyRight:
                        if(right_enabled) {
                            ...
                            FURI_LOG_I(TAG, "droite");
                        }
                        break;
                    case InputKeyLeft:
                        if(left_enabled) {
                            active_signal = left_signal;
                            FURI_LOG_I(TAG, "gauche");
                        }
                        break;
                    case InputKeyOk:
                        if(ok_enabled) {
