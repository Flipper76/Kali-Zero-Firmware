...
bool ibutton_load_key(iButton* ibutton, bool show_error) {
    ...
    if(success) {
        ...
        dialog_message_show_storage_error(ibutton->dialogs, "Impossible de charger\nle fichier key");
    }
...
bool ibutton_save_key(iButton* ibutton) {
    ...
    if(!success) {
        dialog_message_show_storage_error(ibutton->dialogs, "Impossible d'enregistrer\nle fichier key");
    }
...
int32_t ibutton_app(char* arg) {
    ...
    if((arg != NULL) && (strlen(arg) != 0)) {
        if(sscanf(arg, "RPC %lX", (uint32_t*)&ibutton->rpc) == 1) {
            FURI_LOG_D(TAG, "Execution en mode RPC");
