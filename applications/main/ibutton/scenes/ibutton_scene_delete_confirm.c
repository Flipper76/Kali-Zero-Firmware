...
void ibutton_scene_delete_confirm_on_enter(void* context) {
    ...
    widget_add_button_element(widget, GuiButtonTypeLeft, "Retour", ibutton_widget_callback, context);
    widget_add_button_element(
        widget, GuiButtonTypeRight, "Supprimer", ibutton_widget_callback, context);

    furi_string_printf(tmp, "\e#Supprimer %s?\e#", ibutton->key_name);
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        if(event.event == GuiButtonTypeRight) {
            if(ibutton_delete_key(ibutton)) {
                ...
                dialog_message_show_storage_error(ibutton->dialogs, "Impossible de supprimer\nle fichier");
