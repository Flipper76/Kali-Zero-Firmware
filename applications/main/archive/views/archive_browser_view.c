...
static const char* ArchiveTabNames[] = {
    [ArchiveTabFavorites] = "Favoris",
    [ArchiveTabIButton] = "iButton",
    [ArchiveTabNFC] = "NFC",
    [ArchiveTabSubGhz] = "Sub-GHz",
    [ArchiveTabLFRFID] = "RFID",
    [ArchiveTabInfrared] = "Infrarouge",
    [ArchiveTabBadKb] = "Bad KB",
    [ArchiveTabU2f] = "U2F",
    [ArchiveTabApplications] = "Application",
    [ArchiveTabSearch] = "Rechercher",
    [ArchiveTabDiskImage] = "Image Disque",	
    [ArchiveTabInternal] = "Interne",
    [ArchiveTabBrowser] = "Parcourir",
};
    ...
    if(menu_array_size(model->context_menu) == 0) {
        ...
        if(model->menu_manage) {
            ...
            if(archive_is_known_app(selected->type)) {
                ...
                archive_menu_add_item(
                    menu_array_push_raw(model->context_menu),
                    (selected->fav || favorites) ? "Suppr. fav" : "Favoris",
            ...
            if(!selected->is_app) {
                ...
                if(selected->type != ArchiveFileTypeFolder) {
                    archive_menu_add_item(
                        menu_array_push_raw(model->context_menu),
                        selected->type == ArchiveFileTypeDiskImage ? "Monter" : "Afficher",						
...
static void archive_render_status_bar(Canvas* canvas, ArchiveBrowserViewModel* model) {
    ...
    if(clip) {
        ...
        canvas_draw_str_aligned(
            canvas, 81, 9, AlignCenter, AlignBottom, model->clipboard_copy ? "Copier" : "Couper");
    }
...
static void archive_view_render(Canvas* canvas, void* mdl) {
    ...
    if(model->folder_loading) {
        ...
        canvas_draw_str_aligned(
            canvas, GUI_DISPLAY_WIDTH / 2, 40, AlignCenter, AlignCenter, "Vide");
        if(model->menu) {
            render_item_menu(canvas, model);
        }
    }
