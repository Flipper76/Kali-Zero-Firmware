    ...
    if(dir_walk_open(dir_walk, furi_string_get_cstr(current->path))) {
        while(scene_manager_get_scene_state(instance->scene_manager, ArchiveAppSceneInfo)) {
            ...
            if(result == DirWalkError) {
                widget_element_text_box_set_text(instance->element, "Taille: \e#Erreur\e#");
                break;
            }
            ...
            if(!file_info_is_dir(&fileinfo) || is_last) {
                ...
                snprintf(
                    buf,
                    sizeof(buf),
                    unit ? "Taille: %s\e#%.2f\e# %s" : "Taille: %s\e#%.0f\e# %s",
                    is_last ? "" : "... ",
                    show,
                    units[unit]);
        ...
        widget_element_text_box_set_text(instance->element, "Taille: \e#Errer\e#");
    }
...
void archive_scene_info_on_enter(void* context) {
    ...
    widget_add_button_element(
        instance->widget, GuiButtonTypeLeft, "Retour", archive_scene_info_widget_callback, instance);
    ...
    if(storage_common_stat(
       ...
       FSE_OK) {
        snprintf(buf, sizeof(buf), "Taille: \e#Erreur\e#");
    } else if(file_info_is_dir(&fileinfo)) {
        is_dir = true;
        snprintf(buf, sizeof(buf), "Taille: ... \e#0\e# %s", units[0]);
        ...
        snprintf(
            buf,
            sizeof(buf),
            unit ? "Taille: \e#%.2f\e# %s" : "Taille: \e#%.0f\e# %s",
            show,
            units[unit]);
    }
    ...
    // MD5 hash
    if(!is_dir) {
        strlcpy(buf, "MD5: \e*Chargement...", sizeof(buf));
        uint8_t padding = 32 - strlen("Chargement...");
        for(uint8_t i = 0; i < padding; i++) {
            strlcat(buf, " ", sizeof(buf));
        }
        strlcat(buf, "\e*", sizeof(buf));
