...
bool subghz_history_get_text_space_left(
    ...
    if(!ignore_full) {
        if(memmgr_get_free_heap() < SUBGHZ_HISTORY_FREE_HEAP) {
            if(output != NULL) furi_string_printf(output, "    M""\xE9""moire PLEINE");
            return true;
        }
        if(instance->last_index_write == SUBGHZ_HISTORY_MAX) {
            if(output != NULL) furi_string_printf(output, "     Historique PLEIN");
            return true;
        }
    }
    if(output != NULL) {
