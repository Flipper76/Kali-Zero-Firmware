...
uint32_t subghz_frequency_find_correct(uint32_t input) {
    ...
    uint32_t result = 0;
#ifdef FURI_DEBUG
    FURI_LOG_D(TAG, "entrée: %ld", input);
#endif
...
bool subghz_frequency_analyzer_input(InputEvent* event, void* context) {
    ...
    if(((event->type == InputTypePress) || (event->type == InputTypeRepeat)) &&
       ((event->key == InputKeyLeft) || (event->key == InputKeyRight))) {
        ...
        FURI_LOG_I(TAG, "Déclenchement = %.1f", (double)trigger_level);
        need_redraw = true;
                ...
                if(model->show_frame && !model->signal) {
                    ...
                    if(frequency_candidate > 0 &&
                       frequency_candidate != model->frequency_to_save) {
                        ...
                        FURI_LOG_D(
                            TAG,
                            "frequency_to_save : %ld, candidat : %ld",
...
void subghz_frequency_analyzer_pair_callback(
    ...
    if(float_is_equal(rssi, 0.f) && instance->locked) {
        ...
        FURI_LOG_I(TAG, "rssi = %.2f, fréquence = %ld Hz", (double)rssi, frequency);
