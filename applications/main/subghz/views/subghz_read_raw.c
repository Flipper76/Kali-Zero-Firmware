...
void subghz_read_raw_draw(Canvas* canvas, SubGhzReadRAWModel* model) {
    ...
    case SubGhzReadRAWStatusIDLE:
        elements_button_left(canvas, "Effacer");
        elements_button_center(canvas, "Envoyer");
        elements_button_right(canvas, "Enregistrer");
        break;
    case SubGhzReadRAWStatusLoadKeyIDLE:
        if(!model->raw_send_only) {
            elements_button_left(canvas, "Nouveau");
            elements_button_right(canvas, "Plus");
        }
        elements_button_center(canvas, "Evoyer");
        elements_text_box(
            canvas,
            4,
            20,
    ...
    case SubGhzReadRAWStatusLoadKeyTXRepeat:
        graphics_mode = 0;
        elements_button_center(canvas, "Maintenez pour r""\xE9""p""\xE9""ter");
        break;
    ...
    default:
        elements_button_center(canvas, "Stoper");
        break;
    }
