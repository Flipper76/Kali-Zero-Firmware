...
static void infrared_scene_rpc_show(InfraredApp* infrared) {
    ...
    popup_set_header(popup, "Infrarouge", 89, 42, AlignCenter, AlignBottom);
    popup_set_text(popup, "mode RPC", 89, 44, AlignCenter, AlignTop);
        ...
        if(event.event == InfraredCustomEventTypeRpcLoadFile) {
            ...
            if(rpc_state == InfraredRpcStateLoaded) {
                if(event.event == InfraredCustomEventTypeRpcButtonPressName) {
                    ...
                    FURI_LOG_D(TAG, "Envoi signal avec le nom \"%s\"", button_name);
                } else {
                    FURI_LOG_D(
                        TAG, "Envoi signal avec index \"%ld\"", app_state->current_button_index);						
                }
                if(infrared->app_state.current_button_index != InfraredButtonIndexNone) {
                    ...
                    if(!INFRARED_ERROR_PRESENT(error)) {
                        const char* remote_name = infrared_remote_get_name(infrared->remote);
                        infrared_text_store_set(infrared, 0, "emulation\n%s", remote_name);
                        ...
                        rpc_system_app_set_error_text(
                            infrared->rpc_ctx, "Impossible de charger \nles données bouton");
                        result = false;
                    }
                }
            }
