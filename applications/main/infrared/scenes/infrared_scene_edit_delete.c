...
void infrared_scene_edit_delete_on_enter(void* context) {
	...
	if(edit_target == InfraredEditTargetButton) {
        dialog_ex_set_header(dialog_ex, "Supprimer Bouton?", 64, 0, AlignCenter, AlignTop);
    ...
    dialog_ex_set_right_button_text(dialog_ex, "Supprimer");
        ...
        if(event.event == DialogExResultLeft) {
            ...
            if(task_success) {
                ...
                const char* edit_target_text =
                    app_state->edit_target == InfraredEditTargetButton ? "bouton" : "fichier";
                infrared_show_error_message(infrared, "Échec de \nsuppression %s", edit_target_text);
