...
static void dict_attack_draw_callback(Canvas* canvas, void* model) {
    ...
    if(!m->card_detected) {
        ...
        canvas_draw_str_aligned(canvas, 64, 4, AlignCenter, AlignTop, "Tag perdu!");
        ...
        snprintf(
            draw_str, sizeof(draw_str), "Secteurs lus: %d/%d", m->sectors_read, m->sectors_total);
        canvas_draw_str_aligned(canvas, 0, 43, AlignLeft, AlignTop, draw_str);
    }
    elements_button_center(canvas, "Passer");
}
