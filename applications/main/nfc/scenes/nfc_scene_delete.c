...
void nfc_scene_delete_on_enter(void* context) {
    ...
    furi_string_printf(temp_str, "\e#Supprimer %s?\e#", furi_string_get_cstr(nfc->file_name));
    widget_add_text_box_element(
        nfc->widget, 0, 0, 128, 23, AlignCenter, AlignCenter, furi_string_get_cstr(temp_str), false);
    widget_add_button_element(
        nfc->widget, GuiButtonTypeLeft, "Annuler", nfc_scene_delete_widget_callback, nfc);
    widget_add_button_element(
        nfc->widget, GuiButtonTypeRight, "Supprimer", nfc_scene_delete_widget_callback, nfc);
