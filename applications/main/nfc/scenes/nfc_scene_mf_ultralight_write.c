...
static void nfc_scene_mf_ultralight_write_setup_view(NfcApp* instance) {
    ...
    if(state == NfcSceneMfUltralightWriteStateCardSearch) {
        popup_set_header(instance->popup, "Ecriture", 95, 20, AlignCenter, AlignCenter);
        popup_set_text(
            instance->popup, "Appliquer la carte \ninitiale uniquement", 95, 38, AlignCenter, AlignCenter);
        popup_set_icon(instance->popup, 0, 8, &I_NFC_manual_60x50);
    } else {
        popup_set_header(popup, "Ecriture\nNe bougez pas...", 52, 32, AlignLeft, AlignCenter);
...
void nfc_scene_mf_ultralight_write_on_enter(void* context) {
    ...
    FURI_LOG_D("WMFU", "Recherche de carte...");
