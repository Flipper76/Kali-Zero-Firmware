...
bool mf_classic_key_cache_save(MfClassicKeyCache* instance, const MfClassicData* data) {
    ...
    do {
        ...
        if(!flipper_format_write_string_cstr(
               ff, "Type Mifare Classique", mf_classic_get_device_name(data, NfcDeviceNameTypeShort)))
            break;
        if(!flipper_format_write_hex_uint64(ff, "Map key A", &data->key_a_mask, 1)) break;
        if(!flipper_format_write_hex_uint64(ff, "Map key B", &data->key_b_mask, 1)) break;
        ...
        for(size_t i = 0; (i < sector_num) && (key_save_success); i++) {
            ...
            if(FURI_BIT(data->key_a_mask, i)) {
                furi_string_printf(temp_str, "Secteur key A %d", i);
            ...
            if(FURI_BIT(data->key_b_mask, i)) {
                furi_string_printf(temp_str, "Secteur key B %d", i);
                key_save_success = flipper_format_write_hex(
...
bool mf_classic_key_cache_load(MfClassicKeyCache* instance, const uint8_t* uid, size_t uid_len) {
    ...
    do {
        ...
        if(!flipper_format_read_hex_uint64(ff, "Map key A", &instance->keys.key_a_mask, 1)) break;
        if(!flipper_format_read_hex_uint64(ff, "Map key B", &instance->keys.key_b_mask, 1)) break;
