...
static void bad_kb_draw_callback(Canvas* canvas, void* _model) {
    ...
    if((state == BadKbStateIdle) || (state == BadKbStateDone) ||
       (state == BadKbStateNotConnected)) {
            elements_button_center(canvas, "Lancer");
        ...
        elements_button_center(canvas, "Fin");
        elements_button_right(canvas, "Reprendre");
    } else if(state == BadKbStateWaitForBtn) {
        elements_button_center(canvas, "Appuyez pour continuer");
    } else if(state == BadKbStateWillRun) {
        elements_button_center(canvas, "Annuler");
    }
    ...
    if(state == BadKbStateNotConnected) {
        ...
        canvas_draw_icon(canvas, 4, 26, &I_Clock_18x18);
        canvas_set_font(canvas, FontPrimary);
            canvas_draw_str_aligned(canvas, 127, 31, AlignRight, AlignBottom, "Se lance");
        canvas_draw_str_aligned(canvas, 127, 43, AlignRight, AlignBottom, "Connexion");
    } else if(state == BadKbStateFileError) {
        canvas_draw_icon(canvas, 4, 26, &I_Error_18x18);
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 127, 31, AlignRight, AlignBottom, "Fichier");
        canvas_draw_str_aligned(canvas, 127, 43, AlignRight, AlignBottom, "ERREUR");
    } else if(state == BadKbStateScriptError) {
        canvas_draw_icon(canvas, 4, 26, &I_Error_18x18);
        furi_string_printf(disp_str, "ligne %zu", model->state.error_line);
        ...
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 127, 33, AlignRight, AlignBottom, "ERREUR:");
        ...
        if(delay) {
            furi_string_printf(disp_str, "Retarder %lus", delay);
            canvas_draw_str_aligned(
        ...
        if(state != BadKbStateWaitForBtn) {
            canvas_draw_str_aligned(canvas, 4, 61, AlignLeft, AlignBottom, "Pause");
        }
        furi_string_printf(disp_str, "%zu/%zu", model->state.line_cur, model->state.line_nb);
        canvas_draw_str_aligned(
            canvas, 124, 47, AlignRight, AlignBottom, furi_string_get_cstr(disp_str));
