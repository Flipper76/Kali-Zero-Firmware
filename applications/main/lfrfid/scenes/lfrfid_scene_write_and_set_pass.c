...
void lfrfid_scene_write_and_set_pass_on_enter(void* context) {
    ...
    popup_set_header(popup, "Ecriture\navec mdp", 89, 30, AlignCenter, AlignTop);
...
bool lfrfid_scene_write_and_set_pass_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == LfRfidEventWriteOK) {
            ...
            popup_set_header(popup, "Erreur", 64, 3, AlignCenter, AlignTop);
            popup_set_text(popup, "Ecriture du protocole \nimpossible", 3, 17, AlignLeft, AlignTop);
