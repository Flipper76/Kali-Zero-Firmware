...
void power_draw_battery_callback(Canvas* canvas, void* context) {
    furi_assert(context);
    Power* power = context;
    BatteryIcon battery_icon = kalizero_settings.battery_icon;
...
Power* power_alloc() {
    ...
    power_set_battery_icon_enabled(power, kalizero_settings.battery_icon != BatteryIconOff);
...
static void power_check_charge_cap(Power* power) {
    uint32_t cap = kalizero_settings.charge_cap;
