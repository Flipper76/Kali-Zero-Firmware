...
static void rpc_system_gui_virtual_display_render_callback(Canvas* canvas, void* context) {
    ...
    if(!rpc_gui->virtual_display_not_empty) {
        ...
        canvas_draw_str_aligned(canvas, 64, 20, AlignCenter, AlignCenter, "Affichage virtuel");
        canvas_draw_str_aligned(canvas, 64, 36, AlignCenter, AlignCenter, "En attente des images...");
        return;
    }
