...
void desktop_scene_lock_menu_save_settings(Desktop* desktop) {
    ...
    if(desktop->lock_menu->save_kalizero) {
        kalizero_settings_save();
        desktop->lock_menu->save_kalizero = false;
    }
...
bool desktop_scene_lock_menu_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        case DesktopLockMenuEventLockPinOff:
            ...
            consumed = true;
            break;
        case DesktopLockMenuEventKaliZero:
            desktop_scene_lock_menu_save_settings(desktop);
            loader_start_detached_with_gui_error(desktop->loader, "KaliZero", NULL);
            consumed = true;
            break;
