...
static bool
    ...
    FURI_LOG_I(TAG, "données supprimées du tampon");
...
static bool
    ...
    FURI_LOG_I(TAG, "Données reçues: %s", furi_string_get_cstr(tmp_str));
    ...
    if(event.type == InputTypeLong) {
        ...
        if(event.key == InputKeyBack) {
            if(!furi_event_loop_timer_is_running(app->exit_timer)) {
                ...
                FURI_LOG_I(TAG, "Démarrage du compte à rebours de sortie!");
            ...
            if(furi_event_loop_timer_is_running(app->exit_timer)) {
                ...
                FURI_LOG_I(TAG, "Compte à rebours de sortie annulé!");
                ...
                FURI_LOG_W(TAG, "Le compte à rebours n'a pas encore commencé");
            }

        } else {
            // Not a Back or Ok key, just print its name.
            FURI_LOG_I(TAG, "Appui long: %s", input_get_key_name(event.key));
        }

    } else if(event.type == InputTypeShort) {
        // Not a long press, just print the key's name.
        FURI_LOG_I(TAG, "Appui court: %s", input_get_key_name(event.key));
    }
...
static void event_loop_multi_app_exit_timer_callback(void* context) {
    ...
    FURI_LOG_I(TAG, "Sortie dans %lu ...", app->exit_countdown_value);

    // If the coundown value has reached 0, exit the application
    if(app->exit_countdown_value == 0) {
        FURI_LOG_I(TAG, "Sortie MAINTENANT!");
...
static void event_loop_multi_app_run(EventLoopMultiApp* app) {
    FURI_LOG_I(TAG, "Appuyez sur les touches pour les voir imprimées ici.");
    FURI_LOG_I(TAG, "Appuyez longuement sur \"Retour\" pour quitter après %lu secondes.", COUNTDOWN_START_VALUE);
    FURI_LOG_I(TAG, "Appuyez longuement sur \"Ok\" pour annuler le compte à rebours.");
