...
void desktop_settings_scene_happy_mode_on_enter(void* context) {
    ...
    dialog_ex_set_header(app->dialog_ex, "Mode Joyeux", 64, 0, AlignCenter, AlignTop);
    dialog_ex_set_text(
        app->dialog_ex,
        "Jamais je serais fâché\ncontre toi si tu ne\nreste pas avec moi\nsi le mode est activé",
        64,
        30,
        AlignCenter,
        AlignCenter);
    dialog_ex_set_left_button_text(app->dialog_ex, settings.happy_mode ? "Désactiver" : "Retour");
    dialog_ex_set_right_button_text(
        app->dialog_ex, settings.happy_mode ? "Garder activé" : "Activer");
