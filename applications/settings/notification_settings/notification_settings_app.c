...
static NotificationAppSettings* alloc_settings() {
    ...
    item = variable_item_list_add(
        app->variable_item_list, "Contraste LCD", CONTRAST_COUNT, contrast_changed, app);
    value_index =
    ...
    if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagStealthMode)) {
        ...
        value_index = 0;
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, "Furtif");
    } else {
        item = variable_item_list_add(
    ...
    if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagStealthMode)) {
        item = variable_item_list_add(app->variable_item_list, "Vibreur", 1, NULL, app);
        value_index = 0;
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, "Furtif");
    } else {
        item = variable_item_list_add(
            app->variable_item_list, "Vibreur", VIBRO_COUNT, vibro_changed, app);
        value_index =
